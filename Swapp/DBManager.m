...
//Not a NSString, because SQL can only reconize C string.
...
-(void)runQuery:(const char *)query isQueryExecutable:(BOOL)queryExecutable{
    ...
    //Initialize teh column names array.
    ...
    if(prepareStatementResult == SQLITE_OK){
        ...
        if (!queryExecutable) {
            ...
            //Declare on array to keep the data for each fetched row.
            NSMutableArray *arrDataRow;
            
            //Loop through the results and add them to the results array row by row.
            while (sqlite3_step(compiledStatement) == SQLITE_ROW){
                //Initialize the mutable array that will contain the data of a fetch row.
...
-(NSArray *)loadDataFromDB:(NSString *)query{
    ...
    //The query string is convert to a char* object.
